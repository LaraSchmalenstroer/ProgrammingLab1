...
def global_alignment(seqA,seqB,d,e,blosum):    
    if d>0:
        d=(-d)
    if e>0:
        e=(-e)
    ...
    gaps=np.zeros((len(seqA)+1,len(seqB)+1),dtype=int)
    ...
    for i in range(1,len(seqA)+1):
        for j in range(1,len(seqB)+1):
            ...
            if gaps[i,j-1]==-1:
                ...
                gap_2=al[i,j-1]+d
            match=al[i-1,j-1]+blosum[seqA[i-1]][seqB[j-1]]
            al[i,j]=max(match,gap_1,gap_2) #if multiple elements have the same value, the first one is picked
            if al[i,j]==match:
                tb[i,j]=[i-1,j-1]
                gaps[i,j]=0
            elif al[i,j]==gap_1:
...
d=int(sys.argv[3])
e=int(sys.argv[4])
blosum=get_scoring_matrix(sys.argv[5])
score,alignment=global_alignment(seqA,seqB,d,e,blosum)
